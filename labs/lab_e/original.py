...
def update_contour():
    ...
    if image is None:
        ...
        #Cropping
        image = rc_utils.crop(image, (150,0), (rc.camera.get_height(), rc.camera.get_width()))
        ...
        maskORANGE = cv.inRange(hsv, ORANGE[0], ORANGE[1])

        #Find contours
        contoursBLUE, _ = cv.findContours(maskBLUE, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
        contoursGREEN, _ = cv.findContours(maskGREEN, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
        contoursRED, _ = cv.findContours(maskRED, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
        contoursYELLOW, _ = cv.findContours(maskYELLOW, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
        contoursPURPLE, _ = cv.findContours(maskPURPLE, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
        contoursORANGE, _ = cv.findContours(maskORANGE, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)

        #Get largest contours
        rc_utils.get_largest_contour(contoursBLUE, MIN_CONTOUR_AREA)
        rc_utils.get_largest_contour(contoursGREEN, MIN_CONTOUR_AREA)
        rc_utils.get_largest_contour(contoursRED, MIN_CONTOUR_AREA)
        rc_utils.get_largest_contour(contoursYELLOW, MIN_CONTOUR_AREA)
        rc_utils.get_largest_contour(contoursPURPLE, MIN_CONTOUR_AREA)
        rc_utils.get_largest_contour(contoursORANGE, MIN_CONTOUR_AREA)

        #Draw contours
        cv.drawContours(image, contoursBLUE, -1, (0, 0, 0), 3)
        cv.drawContours(image, contoursGREEN, -1, (0, 0, 0), 3)
        cv.drawContours(image, contoursRED, -1, (0, 0, 0), 3)
        cv.drawContours(image, contoursYELLOW, -1, (0, 0, 0), 3)
        cv.drawContours(image, contoursPURPLE, -1, (0, 0, 0), 3)
        cv.drawContours(image, contoursORANGE, -1, (0, 0, 0), 3)
    ...
    largest_contour_blue = rc_utils.get_largest_contour(contoursBLUE, MIN_CONTOUR_AREA)
    
    if largest_contour_blue is not None:
        areaBLUE = cv.contourArea(largest_contour_blue)
        print("Blue contour area:", areaBLUE)
    ...
    print("Stoplight color:", stoplight_color)
    markers = rc_utils.get_ar_markers(image)
    for marker in markers:
        marker.detect_colors(image, [BLUE, GREEN, RED, YELLOW, ORANGE, PURPLE])
        marker.get_color()
    #draw_ar_markers function
