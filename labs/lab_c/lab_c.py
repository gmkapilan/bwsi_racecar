...
def start():
    ...
    speed_offset = 0.0 # The initial speed offset is 0.5
    angle_offset = 0.0 # The inital angle offset is 1.0
...
def update():
    ...
    if rc.controller.was_pressed(rc.controller.Button.A) and speed_offset < 0.5:
        speed_offset += 0.1
        # print(f"The speed_offset is: {speed_offset}")
        print(f"The current speed is: {speed}")
    elif rc.controller.was_pressed(rc.controller.Button.B) and speed_offset > -0.5:
        speed_offset -= 0.1
        # print(f"The speed_offset is: {speed_offset}")
        print(f"The current speed is: {speed}")
    ...
    if rc.controller.was_pressed(rc.controller.Button.X) and angle_offset < 0.5:
        angle_offset += 0.1
        # print(f"The angle_offset is: {angle_offset}")
        print(f"The current angle is: {angle}")
    elif rc.controller.was_pressed(rc.controller.Button.Y) and angle_offset > -0.5:
        angle_offset -= 0.1
        # print(f"The angle_offset is: {angle_offset}")
        print(f"The current angle is: {angle}")

    if speed == 1.0 and rc.controller.was_pressed(rc.controller.Button.A):
        print("Maximum speed forward!")
    if speed == -1.0 and rc.controller.was_pressed(rc.controller.Button.B):
        print("Maximum speed backward!")
    if angle == 1.0 and rc.controller.was_pressed(rc.controller.Button.X):
        print("Maximum angle to the right!")
    if angle == -1.0 and rc.controller.was_pressed(rc.controller.Button.Y):
        print("Maximum angle to the left!") 
